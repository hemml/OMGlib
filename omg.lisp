  ...
  (labels ((is-f-class (cls)
    ...
    (let* ((f-super (remove-if-not #'is-f-class superclasses))
      ...
      `(progn
         ...
         ,@(mapcar
             (lambda (slt)
               `(defmethod-r ,(intern (format nil "OMG-M-READER-~A-~A" name (car slt)) :omg)
                             ((obj ,name))
                  (slot-value obj ',(car slt))))
             m-slots)
                     ...
                     (add ',(mapcar
                              (lambda (slt)
                                `(setf (slot-value var ',(car slt))
                                       (,(intern (format nil "OMG-M-READER-~A-~A" name (car slt)) :omg) var)))
                              m-slots)))
  ...
  (let ((uri (getf env :REQUEST-URI))
    ...
    (cond ((equal uri (concatenate 'string *root-path* *service-worker-path*))
          ...
          ((and (equal uri (concatenate 'string *root-path* *rpc-path*))
           ...
           (let* ((cmd (omg-read-from-string (omg::replace-all (get-str-from (getf env :raw-body) (getf env :content-length))
                                                               "\\n"
