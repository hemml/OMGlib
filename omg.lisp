  ...
  (let* ((sem-dat-sym (gethash-lock key *takit-wait-list*)))
    (if (and sem-dat-sym (assoc :sem sem-dat-sym))
      (let ((newsem (make-semaphore)))
        ...
        (if (wait-on-semaphore newsem :timeout 600)
   ...
   (let* ((sem (make-semaphore))
      ...
      (if (wait-on-semaphore sem :timeout 600)
          (let ((res (cdr (assoc :result (gethash-lock key *gimme-wait-list*)))))
             ...
             (unintern key)
            `(200 (:content-type "text/plain; charset=utf-8") (,res)))
          `(500 (:content-type "text/plain; charset=utf-8") ("Server timeout")))))
                         ...
                         (progn
                           ...
                           (if (not nowait)
                               (labels ((get-res (&key (timeout *remote-exec-timeout*) retry)
                                          (if (wait-on-semaphore sem :timeout (min timeout timeout-chunk))
